...
const Day = () => {
    ...
    // Default Delhi
    useEffect(() => {
        getWeather(28.6139, 77.2090, 'Delhi', 'IN');
    }, []);
    ...
    const getForecast = async (lat, lon) => {
        try {
            ...
            // Convert to array with min/max per day
            const dailyForecast = Object.keys(daily).map(date => {
                ...
                return {
                    ...
                    icon: icons[0] // just pick the first icon of the day
                };
            });

            return dailyForecast.slice(0, 5); // only 5 days
        } catch (err) {
            console.log(err);
        }
    };
